...
class ChatPresenter:
    ...
    def present(canonical_response: dict, execution_id: str, processing_time_ms: int = 0) -> ChatResponse:
        ...
        # Narrative response and thinking trace are 100% generated by the LLM Reasoning Agent
        answer = canonical_response.get("final_answer") or canonical_response.get("content")
        thought = canonical_response.get("thought") or canonical_response.get("thinking_process")
        ...
        # Fallback if and only if LLM did not generate an answer
        if not answer:
            if target_color:
            ...
            elif is_suspect_query and evidence:
                answer = f"Based on forensic analysis of the CCTV footage, I identified the primary suspect as **Person {evidence[0].person_id}**."
            else:
                answer = f"I analyzed the CCTV footage and identified **{person_count} unique individual(s)**."
